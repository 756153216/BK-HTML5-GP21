...
function createServer() {
    ...
    server.listen(4010, "10.9.72.237", listenHandler);
}
    ...
    req.on("end", function () {
        ...
        inquiredata('user', data).then((res) => {
            if (res.length) {
                ...
                o = '未注册'
                // console.log(o)
                re.end(`<script language="javascript" type="text/javascript">
                
                var Days = 7;
                var exp = new Date();
                exp.setTime(exp.getTime() + Days * 24 * 60 * 60 * 1000);
                document.cookie ='flag' + "=" + escape('0') + ";expires=" + exp.toGMTString();
             
               
                window.location.href="http://10.9.72.237:5500/index.html"; 
            </script>`)
                // 
            }
        })
...
function database(_type, _data) {
    ...
    var a = f.user
    var b = f.password
    // var str = 'INSERT INTO "' + table + '" ( `user`, `password`) VALUES ("' + f.user + '","' + f.password + '")'
    var str = "INSERT INTO `" + table + "` (`user`, `password`) VALUES (" + a + ",'" + b + "')"
...
function inquiredata(_type, _data) {
    ...
    var str = "SELECT * FROM " + table + " WHERE user = '" + a + "'&& password='" + b + "'"
...
function herf(_res) {
    console.log(data)
    var name = _res[0].user;
    var value = _res[0].password;
